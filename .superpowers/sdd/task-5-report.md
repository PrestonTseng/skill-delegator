...
## Task 6 final changed-line regression closure
...
- Config evidence now traverses from the filesystem anchor with descriptor-relative
  `O_DIRECTORY | O_NOFOLLOW` opens, opens each final file relative to its retained parent,
  compares file and parent identities around bounded reads, and bounds every descriptor-close
  failure. Repository provenance is unavailable unless the lexical config directory and all
  files still match the retained inode evidence.
- Receipt publication retains a separate anchored cleanup descriptor. Any temporary cleanup or
  primary descriptor-close failure after a new hard-link publication rolls back that new JSON
  name and fsyncs through the retained descriptor before raising. A receipt that existed before
  the call is never removed. The final cleanup-descriptor close is explicitly post-commit and
  cannot turn a successful publication into a reported failure.
- Receipt semantic validation now requires canonical unique target fingerprint IDs, fingerprint
  count coherence with `desired_targets`, exact reason/category counts, and—for converged
  evidence—zero findings/counts plus `verified_links == desired_links`. Legitimate partial drift
  remains publishable.

### TDD evidence

The three public regressions were added before production changes. The focused RED run failed
`3` tests for the expected missing keyword/ancestor traversal, retained JSON after a reported
cleanup failure, and accepted impossible converged evidence. The same focused run passed `3`
tests after implementation.

### Fresh verification

```text
uv run --python 3.12 pytest -q tests/unit/test_verifier.py tests/unit/test_receipts.py tests/integration/test_verify_status_cli.py
41 passed in 0.66s
...
Managed-state receipt schema stayed byte-identical to `HEAD` (1,044 bytes, SHA-256
`e9db28c6603bf7aedf47934308820a8b757db3ecfe03f4b3b60baf46629d03ee`). Verification schema
source, wheel, and sdist copies were identical (4,849 bytes, SHA-256
`52b26b9e5552fa119fab11e9d42f6b97148de39d34bd05addf7f1f2d1c2beb4f`). The reviewer probe
also reported no external config bytes hashed, zero published JSON after the injected receipt
error, and rejection of impossible converged evidence.

Scope remained bounded to Task 6 verifier/receipt regressions, tests, and this report. No
checked-in config apply, Task 7+, network operation, push, merge, or `/opt/knowledge` change was
made.
